...
function App() {
  ...
  const handleClick = async (event) => {
    ...
    if(ipPattern.test(ipAddress)) {
      // axios.defaults.withCredentials = true;
      let a = await axios.get(`https://ip-tracker-server.vercel.app/?data=${encodeURIComponent(userIPAddress)}`);
      let r = await a.json();
      setIpTracker(r);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      console.log(response.data.ip);
      setIpComplete(true);
      let a = await axios.get(`https://ip-tracker-server.vercel.app/?data=${encodeURIComponent(response.data.ip)}`);
      let r = await a.json();
      setIpTracker(r);
      console.log(r);
